...
def grid_vect(clf, parameters_clf, X_train, X_test, parameters_text=None, vect=None, is_w2v=False):
    ...
    if is_w2v:
        ...
        for i in range(SIZE):
            w2vcols.append(i)
        features = FeatureUnion([('textcounts', ColumnExtract(cols=textcountscols))
                                 , ('w2v', ColumnExtract(cols=w2vcols))]
                                , n_jobs=-1)
    else:
        features = FeatureUnion([('textcounts', ColumnExtract(cols=textcountscols))
                                 , ('pipe', Pipeline([('cleantext', ColumnExtract(cols='clean_text')), ('vect', vect)]))]
                                , n_jobs=-1)
...
,'count_mentions','count_urls','count_words']
features = FeatureUnion([('textcounts', ColumnExtract(cols=textcountscols))
, ('pipe', Pipeline([('cleantext', ColumnExtract(cols='clean_text'))
...
new_positive_tweets = pd.Series(["Starting Monday, our great Farmers can begin doing business again with Mexico and Canada. They have both taken the tariff penalties off of your great agricultural product. Please be sure that you are treated fairly. Any complaints should immediately go to  @SecretarySonny Perdue!"
,"Love flying with you guys ask these years. Sad that this will be the last trip 😂 @VirginAmerica #LuxuryTravel"
,"Wow @VirginAmerica main cabin select is the way to fly!! This plane is nice and clean & I have tons of legroom! Wahoo! NYC bound! ✈️", "Tensions between Canada and the US increase as Trump states his intention to escalate the trade war"])
